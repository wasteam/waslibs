...
namespace AppStudio.Uwp.Converters
{
    public class BoolToSizeConverter : IValueConverter
    {
        ...
        {
            if (value == null || string.IsNullOrEmpty(value.ToString())) return (Double)0;
            bool boolInput = false;
            bool castSuccess = Boolean.TryParse(value.ToString(), out boolInput);
            if (!castSuccess) return (Double)0;
            if (parameter == null)
            {
                if (boolInput)
                {
